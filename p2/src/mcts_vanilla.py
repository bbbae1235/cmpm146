...
def traverse_nodes(node: MCTSNode, board: Board, state, bot_identity: int):
    ...
    """
    best, best_action = get_best_action(node, False)
    
    while best_action is not None and not board.is_ended(state):
    ...
    '''
    # get best expandable node and best action
    best, best_action = get_best_action(node)
    
    # if there was no child with untried actions or a terminal node was reached then just return the root node
    if (board.is_ended(state) or best == None):
        return node, state
    
    # set the new state to the state after the best action is applied
    new_state = board.next_state(state, best_action)
    # traverse the subtree where the root is child with the largest uct
    # if it was just the opponent's turn then make it the bot's turn and vice versa
    if (bot_identity == 1):
        traverse_nodes(best, board, new_state, 2)
    else:
        traverse_nodes(best, board, new_state, 1)
'''
...
def think(board: Board, current_state):
    ...
    """
    #initialize variables: identity and root
    bot_identity = board.current_player(current_state) # ID is current player 1 or 2
    #root should have no parent 
    root_node = MCTSNode(parent=None, parent_action=None, action_list=board.legal_actions(current_state)) #assign actions to root
